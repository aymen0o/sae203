...
if ($formulaire_soumis) {
    ...
    if (empty(trim($nom)) || empty(trim($prenom))) {
        $message_erreur = "Le nom et le prénom ne peuvent pas être vides.";
        
        $entite['nom'] = $nom;
        $entite['prenom'] = $prenom;
        $entite['lien_avatar'] = $lien_avatar;
        $entite['lien_twitter'] = $lien_twitter;
    } else {
        $requete_brute = "
            UPDATE auteur
            SET
                    ...
                    <form method="POST" action="" class="rounded-lg bg-white p-4 shadow border-gray-300 border-1">
                        <section class="grid gap-6">
                            ...
                            <div class="mb-3 col-md-6">
                                <button type="submit" class="rounded-md bg-indigo-600 py-2 px-4 text-lg font-medium text-white shadow-sm hover:bg-indigo-700 focus-within:bg-indigo-700 transition duration-300">Enregistrer les modifications</button>
                            </div>
                        </section>
                    </form>
                <?php } else { ?>
